/* ============================================================
   GOLAZO — Lobbies · Create · Lobby detail (chat + borrow)
   ============================================================ */

/* ===================== LOBBIES LIST ===================== */
function LobbyCard({ l, s, joined }) {
  return (
    <div className="card card-pad card-hover pointer" onClick={() => s.go('lobby', { id: l.id })}>
      ...
      <div className="row between">
        ...
        {joined
          ...
          : <Btn variant="primary" size="sm" onClick={(e) => { e.stopPropagation(); s.toastMsg(`Joined ${l.name}!`, 'check', 'var(--green)'); s.go('lobby', { id: l.id }); }}>Join</Btn>}
      </div>
    </div>
  );
}
...
function Lobbies({ s }) {
  ...
  const match = (l) => (l.name + l.owner + l.scope).toLowerCase().includes(q.toLowerCase());
  ...
  return (
    <div className="page fade-up">
      ...
      {/* search + join */}
      <div className="grid gap-12" style={{ gridTemplateColumns: 'minmax(0,1.4fr) minmax(0,1fr)', marginBottom: 22 }} id="lobby-tools">
...
/* ===================== CREATE LOBBY ===================== */
function LobbyCreate({ s }) {
  const [borrow, setBorrow] = useState(true);
  const [def, setDef] = useState(1000);
  // pool of selectable matches (live + upcoming first, then a few finished for context)
  const pool = [...WC.live, ...WC.upcoming].slice(0, 20);
  const [sel, setSel] = useState(() => new Set(pool.slice(0, 6).map(m => m.id)));
  const [scope, setScope] = useState('custom');
  const toggle = (id) => { setScope('custom'); setSel(s0 => { const n = new Set(s0); n.has(id) ? n.delete(id) : n.add(id); return n; }); };

  // tournament-stage quick scopes (knockout rounds have no fixtures until the group draw resolves)
  const stages = [
  ...
  const applyScope = (p) => { setScope(p.k); if (p.future) { setSel(new Set()); } else { setSel(new Set(p.pick())); } };

  const activeStage = stages.find(p => p.k === scope);
  const future = activeStage?.future;
  const count = sel.size;
  const scopeLabel = future ? activeStage.label
...
function LobbyView({ s }) {
  const l = WC.lobbies.find(x => x.id === s.param.id) || WC.lobbies[0];
  const isHost = l.owner === 'You';
  const [tab, setTab] = useState('matches');
  const lobbyMatches = WC.lobbyMatches(l);
  const [odds, setOdds] = useState(() => { const o = {}; lobbyMatches.forEach(m => o[m.id] = { ...m.odds }); return o; });
  const [editM, setEditM] = useState(null);
  const [bets, setBets] = useState({}); // mid -> pick (member bets in this lobby)
  const saveOdds = (id, mh, md, ma) => { setOdds(p => ({ ...p, [id]: { mh, md, ma } })); setEditM(null); s.toastMsg('Lobby odds updated for this match', 'check', 'var(--sky)'); };
  const placeBet = (m, pick) => { setBets(b => ({ ...b, [m.id]: pick })); s.toastMsg(`Lobby bet: ${pick} · uses your lobby wallet`, 'check', 'var(--green)'); };

  const tabs = [['matches', `Matches · ${lobbyMatches.length}`], ['board', 'Standings'], ['chat', 'Chat'], ['requests', 'Requests'], ['members', 'Members']];
    ...
    <div className="page fade-up">
      ...
      <div className="panel card-pad-lg" style={{ background: 'linear-gradient(150deg, var(--surface-2), var(--bg-2))' }}>
        ...
        <div className="row gap-20 mt-16 wrap-w">
          ...
          <div className="stat"><span className="s-val tnum text-sky" style={{ fontSize: 22 }}>{lobbyMatches.length}</span><span className="s-lbl">Matches</span></div>
        </div>
      </div>
      ...
      <div className="mt-16">
        {tab === 'matches' && <LobbyMatches l={l} matches={lobbyMatches} isHost={isHost} odds={odds} bets={bets} onEdit={setEditM} onBet={placeBet} />}
        {tab === 'board' && <LobbyBoard s={s} />}
        {tab === 'chat' && <LobbyChat s={s} />}
        {tab === 'requests' && <LobbyRequests s={s} l={l} isHost={isHost} />}
        {tab === 'members' && <LobbyMembers s={s} l={l} />}
      </div>

      {editM && <LobbyOddsModal m={editM} odds={odds[editM.id]} onClose={() => setEditM(null)} onSave={saveOdds} />}
    </div>
  );
}
    ...
    <div>
      ...
      <div className="stack gap-12">
        {matches.map(m => {
          ...
          const cells = [['1', home.code, o.mh], ['X', 'Draw', o.md], ['2', away.code, o.ma]];
          return (
              ...
              <div className="row gap-8 full">
                {cells.map(([k, lbl, v]) => (
                  <button key={k} className={`odds ${myPick === k ? 'sel' : ''}`} disabled={!open || isHost}
                    onClick={() => open && !isHost && onBet(m, k)} style={(!open || isHost) ? { opacity: isHost ? 1 : .5, cursor: isHost ? 'default' : 'not-allowed' } : null}>
...
function LobbyOddsModal({ m, odds, onClose, onSave }) {
    ...
    <div className="card" style={{ display: 'flex', flexDirection: 'column', height: 460 }}>
      <div className="stack gap-12" style={{ flex: 1, overflowY: 'auto', padding: 18 }}>
        {msgs.map((m, i) => m.who === 'sys'
          ...
          : (
            <div key={i} className={`row gap-10 ${m.who === 'You' ? '' : ''}`} style={{ flexDirection: m.who === 'You' ? 'row-reverse' : 'row', textAlign: m.who === 'You' ? 'right' : 'left' }}>
...
function LobbyRequests({ s, l, isHost }) {
  const [reqs, setReqs] = useState(WC.borrowRequests.map(r => ({ ...r, state: 'pending' })));
  const resolve = (id, state, amt) => {
  ...
  return (
    <div className="stack gap-12">
      ...
      {reqs.map(r => {
        ...
        return (
          <div key={r.id} className="card card-pad" style={{ opacity: decided ? .6 : 1, borderColor: r.repeat && !decided ? 'rgba(255,200,61,.3)' : 'var(--line)' }}>
            <div className="row between wrap gap-12">
...
Object.assign(window, { Lobbies, LobbyCard, LobbyCreate, LobbyView, LobbyMatches, LobbyOddsModal, LobbyRequests, BorrowModal });
