/* ============================================================
   GOLAZO — Landing · Auth · Home
   ============================================================ */

/* ===================== LANDING ===================== */
function Landing({ s }) {
  const feat = [WC.matchById(23), WC.matchById(27), WC.matchById(31)];
  return (
    <div className="landing">
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '40px 24px 20px', textAlign: 'center' }}>
        ...
        <h1 className="display fade-up" style={{ fontSize: 'clamp(40px,8vw,88px)', marginTop: 18, lineHeight: .95 }}>
      ...
      {/* explore — public, no account needed */}
      <div style={{ maxWidth: 1080, margin: '40px auto 0', padding: '0 24px' }}>
        <div className="row between" style={{ marginBottom: 14 }}>
...
/* ===================== AUTH ===================== */
function Auth({ s }) {
  const [mode, setMode] = useState(s.param?.mode || 'signup');
      ...
      <div className="row" style={{ width: '100%', maxWidth: 920, gap: 0, borderRadius: 'var(--r-xl)', overflow: 'hidden', boxShadow: 'var(--sh-3)' }}>
        ...
        <div style={{ flex: 1, background: 'var(--surface)', padding: 'clamp(28px,5vw,44px)', alignSelf: 'stretch' }}>
          ...
          <div className="stack gap-16" style={{ marginTop: 24 }}>
            ...
            <div className="row center gap-12"><div className="hr grow"></div><span className="tiny muted">OR</span><div className="hr grow"></div></div>
...
function Home({ s }) {
  const me = WC.me;
  const today = [WC.matchById(23), WC.matchById(24), WC.matchById(27), WC.matchById(31), WC.matchById(33)];
  const smart = { 23: '1', 27: '2', 31: 'X' };
  return (
    <div className="page fade-up">
      ...
      <div className="grid gap-20" style={{ gridTemplateColumns: 'minmax(0,1.6fr) minmax(0,1fr)' }} id="home-grid">
        ...
        <div>
          <SecHead title="Today's matches" sub="Lock your picks before kickoff"
            action={<button className="chip" onClick={() => s.go('schedule')}>Full schedule →</button>} />
...
function Stat({ val, lbl, c, i, onClick }) {
  return (
...
Object.assign(window, { Landing, Auth, Home });
