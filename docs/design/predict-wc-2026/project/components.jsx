/* ============================================================
   GOLAZO — shared primitives  (exports to window)
   Icon set · Flag crest · Button · Badge · OddsButton ·
   MatchCard · Pundit mascot · Avatar · Sparkline
   ============================================================ */
...
function Icon({ name, size = 20, sw = 1.8, fill, style, className }) {
  const d = ICONS[name] || ICONS.ball;
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" className={className}
      style={style} fill={fill || 'none'} stroke={fill ? 'none' : 'currentColor'}
...
/* ---------------- FLAG CREST ---------------- */
function Flag({ team, size = 32, showCode }) {
  const t = typeof team === 'number' ? WC.byId(team) : team;
  const c = t.colors;
  return (
    <div className="row gap-8" style={{ minWidth: 0 }}>
      <div className="flag" style={{ width: size, height: size }} title={t.name}>
        ...
        <span style={{ background: c[2] }}></span>
      </div>
      {showCode && <span className="flag-code" style={{ fontSize: size * .42 }}>{t.code}</span>}
    </div>
  );
}

/* ---------------- AVATAR ---------------- */
function Avatar({ initials, size = 40, color = 'var(--sky)', ring }) {
  return (
    <div style={{
      width: size, height: size, borderRadius: '50%', flex: 'none',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      background: `linear-gradient(135deg, ${color}, var(--surface-3))`,
      fontFamily: 'var(--f-display)', fontWeight: 800, fontSize: size * .38, color: '#fff',
...
/* ---------------- BUTTON ---------------- */
function Btn({ variant = 'primary', size, icon, iconR, children, className = '', ...p }) {
...
/* ---------------- PUNDIT MASCOT (ORA) ---------------- */
/* A friendly broadcast-owl AI commentator. mood: idle|happy|think */
function Pundit({ size = 56, mood = 'idle', glow }) {
  return (
    <div className="rel" style={{ width: size, height: size, flex: 'none' }}>
      {glow && <div className="abs" style={{ inset: -6, borderRadius: '50%',
        background: 'radial-gradient(circle, rgba(63,192,240,.45), transparent 70%)', filter: 'blur(4px)' }} />}
...
/* ---------------- SPARKLINE ---------------- */
function Spark({ data, w = 80, h = 28, color = 'var(--green)' }) {
...
/* ---------------- ODDS BUTTON ROW ---------------- */
function OddsRow({ m, selected, onPick, compact }) {
    ...
    <div className="row gap-8 full">
      {cells.map(c => (
        <button key={c.k} className={`odds ${selected === c.k ? 'sel' : ''} ${c.val >= 2 ? 'up' : ''}`}
          disabled={locked} onClick={(e) => { e.stopPropagation?.(); onPick && onPick(c.k, c.val); }}
          style={locked ? { opacity: .5, cursor: 'not-allowed' } : null}>
...
/* ---------------- MATCH CARD ---------------- */
function MatchCard({ m, onOpen, onPick, picked, compact }) {
  const home = WC.byId(m.home), away = WC.byId(m.away);
  const live = m.status === 'LIVE', fin = m.status === 'FINISHED';
  return (
    <div className="card card-hover card-pad pointer fade-up" onClick={() => onOpen && onOpen(m)}>
      <div className="row between" style={{ marginBottom: 14 }}>
        <div className="row gap-8">
          ...
          {m.status === 'SCHEDULED' && <span className="small muted">{WC.fmtDate(m.date)} · {m.kickoff}</span>}
        </div>
      ...
      {!compact && (
        <div style={{ marginTop: 14 }}>
          <OddsRow m={m} selected={picked} onPick={onPick} />
        </div>
      )}
    </div>
  );
...
/* ---------------- TIER PILL ---------------- */
const TIER_C = { Legend: '#FF4D8D', Diamond: '#3FC0F0', Platinum: '#9B7DFF', Gold: '#FFC83D', Silver: '#AEB8D0', Bronze: '#c08457' };
function TierPill({ tier }) {
  return <span className="badge" style={{ background: 'transparent', border: `1px solid ${TIER_C[tier]}55`, color: TIER_C[tier] }}>
    <span className="dot" style={{ background: TIER_C[tier] }} />{tier}</span>;
}
...
Object.assign(window, {
  Icon, Flag, Avatar, Btn, Pundit, Spark, OddsRow, MatchCard, SecHead, TierPill, Toast, TIER_C,
});
