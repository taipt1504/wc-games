/* ============================================================
   GOLAZO — WC 2026 data layer (window.WC)
   48 teams · 12 groups (A–L) · fixtures · odds · mock app state
   Flags drawn as color crests (consistent cross-platform).
   ============================================================ */
(function () {
  ...
  const T = [
    ...
    // I
    ['Portugal B','POR2','UEFA',8,['#006600','#ff0000','#ffe900']], // placeholder fixed below
    ...
    // J
    ['Uruguay B','URU2','CONMEBOL',15,['#7bb3e0','#ffffff','#001489']],
    ...
    // K
    ['Colombia B','COL2','CONMEBOL',17,['#fcd116','#003893','#ce1126']],
  ...
  // Fix the two placeholder duplicates with distinct real nations
  T[32] = ['Scotland','SCO','UEFA',39,['#0065bf','#ffffff','#0065bf']];
  T[36] = ['Norway','NOR','UEFA',38,['#ba0c2f','#ffffff','#00205b']];
  T[40] = ['Peru','PER','CONMEBOL',32,['#d91023','#ffffff','#d91023']];

  const GROUPS = 'ABCDEFGHIJKL'.split('');
  const teams = T.map((t, i) => ({
    id: i, name: t[0], code: t[1], conf: t[2], rank: t[3], colors: t[4],
    group: GROUPS[Math.floor(i / 4)],
    w: 0, d: 0, l: 0, gf: 0, ga: 0,
  }));
  const byId = (id) => teams[id];
  const byGroup = (g) => teams.filter(t => t.group === g);
  ...
  // ---- odds helper based on rank gap ----
  function mkOdds(home, away) {
    const gap = (away.rank - home.rank) / 40; // home advantage from rank
    let mh = Math.max(0.35, 1.15 - gap * 0.9 + (Math.random() * .2 - .1));
    let ma = Math.max(0.45, 1.25 + gap * 1.1 + (Math.random() * .25 - .12));
    let md = 0.85 + Math.random() * 0.5;
    const r = x => Math.round(x * 100) / 100;
    return { mh: r(mh), md: r(md), ma: r(ma) };
  }

  // ---- Build round-robin group fixtures (6 per group) + status spread ----
  const RR = [[0,1],[2,3],[0,2],[3,1],[3,0],[1,2]];
  const matches = [];
  let mid = 1;
  const baseDay = new Date('2026-06-11T00:00:00');
  const stadiumKick = ['16:00','19:00','22:00','13:00'];

  GROUPS.forEach((g, gi) => {
    const gt = byGroup(g);
    RR.forEach((pair, mi) => {
      const home = gt[pair[0]], away = gt[pair[1]];
      const dayOffset = gi % 3 + Math.floor(mi / 2) * 3; // spread
      const d = new Date(baseDay); d.setDate(d.getDate() + dayOffset);
      const kick = stadiumKick[(gi + mi) % 4];
      const o = mkOdds(home, away);
      // status: first ~third finished, a couple live, rest scheduled
      let status = 'SCHEDULED', hs = null, as = null;
      const seed = mid;
      if (seed <= 22) { status = 'FINISHED'; hs = Math.floor(Math.random()*4); as = Math.floor(Math.random()*4); }
      else if (seed === 23 || seed === 24) { status = 'LIVE'; hs = Math.floor(Math.random()*3); as = Math.floor(Math.random()*3); }
      matches.push({
        ...
        minute: status === 'LIVE' ? 30 + Math.floor(Math.random()*40) : null,
      });
    });
  });
  ...
  // helper: matches for today / upcoming
  ...
  const leaderboard = lbNames.map((n, i) => ({
    rank: i + 1, name: n, roi: +(64 - i * 4.2 + Math.random() * 2).toFixed(1),
    net: Math.round(12000 - i * 850 + Math.random() * 400),
    settled: 38 - i, won: 30 - i, tier: i < 2 ? 'Legend' : i < 5 ? 'Diamond' : i < 9 ? 'Platinum' : 'Gold',
  ...
  // ---- Lobbies ----  (joined = you're a member · public = discoverable)
  const lobbies = [
    { id: 1, name: 'Office League · ABC Corp', scope: 'Group Stage', members: 14, you: 2, def: 1000,
      owner: 'Khoa Nguyen', borrow: true, pwd: true, hot: true, joined: true, public: false, code: 'ABC123',
      matchIds: [23, 24, 27, 31, 33, 35, 37, 41] },
    { id: 2, name: 'The Lads 🍻', scope: 'Custom · 6 matches', members: 8, you: 1, def: 1500,
      owner: 'You', borrow: true, pwd: true, hot: false, joined: true, public: false, code: 'LADS26',
      matchIds: [27, 31, 33, 35, 41, 45] },
    { id: 3, name: 'Final Four Showdown', scope: 'Whole Tournament', members: 22, you: 5, def: 500,
      owner: 'Trang Le', borrow: false, pwd: false, hot: false, joined: true, public: true, code: 'FINAL4',
      matchIds: [23, 24, 25, 26, 27, 28, 31, 33, 35, 37, 41, 45] },
    // discoverable (not yet joined)
    { id: 4, name: 'Global Pundits Club', scope: 'Whole Tournament', members: 312, you: null, def: 1000,
      owner: 'GolazoHQ', borrow: false, pwd: false, hot: true, joined: false, public: true, code: 'PUNDITS',
      matchIds: [23, 24, 25, 26, 27, 28, 31, 33] },
    { id: 5, name: 'Group Stage Grinders', scope: 'Group Stage', members: 58, you: null, def: 800,
      owner: 'Maria S.', borrow: true, pwd: false, hot: false, joined: false, public: true, code: 'GRIND8',
      matchIds: [23, 24, 27, 31, 33, 35] },
    { id: 6, name: 'Underdog Hunters', scope: 'Custom · 5 matches', members: 41, you: null, def: 1200,
      owner: 'Diego R.', borrow: true, pwd: false, hot: false, joined: false, public: true, code: 'UNDERD',
      matchIds: [27, 31, 35, 41, 45] },
  ];
  function lobbyMatches(l) {
    return (l.matchIds || []).map(id => matches.find(m => m.id === id)).filter(Boolean);
  }
  const lobbyBoard = [
  ...
  // ---- Pending borrow requests (host approval queue) ----
  ...
  // ---- News ----
  const news = [
    { id: 1, tag: 'Match Preview', title: 'Argentina look to seal top spot against Denmark', src: 'GoalWire',
      time: '2h ago', excerpt: 'Messi rested but Álvarez in red-hot form as the holders eye an early knockout berth.', hot: true, match: 14 },
    { id: 2, tag: 'Squad News', title: 'France confirm Mbappé fit to start Group E opener', src: 'KickReport',
      time: '4h ago', excerpt: 'A late fitness test cleared the captain after a minor knock in training Thursday.', match: 17 },
    { id: 3, tag: 'Analysis', title: 'Why the 48-team format is reshaping group-stage tactics', src: 'The Tactical',
      time: '6h ago', excerpt: 'Three-team math is gone — coaches now chase goal difference from minute one.' },
    { id: 4, tag: 'Transfer Buzz', title: 'Breakout star already linked with three European giants', src: 'MercatoDaily',
      time: '8h ago', excerpt: 'Scouts packed the stands as the 19-year-old ran the show in matchday one.' },
    { id: 5, tag: 'Result', title: 'Spain edge Germany in instant Group H classic', src: 'GoalWire',
      time: '12h ago', excerpt: 'A late winner settles a six-goal thriller that lit up the tournament.' },
    { id: 6, tag: 'Off-pitch', title: 'Three-nation host buzz: fans flock across the borders', src: 'FanZone',
      time: '1d ago', excerpt: 'Record cross-border travel as the first multi-host World Cup kicks into gear.' },
  ];

  // ---- Admin: risk-flagged lobbies ----
  const riskLobbies = [
    { id: 91, name: 'private_room_x', members: 2, risk: 'High', score: 87,
      reasons: ['One-way point flow', 'Shared IP (2/2 accounts)', 'Repeated max borrows'], flagged: '12m ago' },
    { id: 92, name: 'wknd crew', members: 3, risk: 'Medium', score: 61,
      reasons: ['Unusual borrow volume', 'Two accounts same device'], flagged: '1h ago' },
    { id: 93, name: 'Sunday Stakes', members: 6, risk: 'Low', score: 34,
      reasons: ['Borrow spike at kickoff'], flagged: '3h ago' },
  ];
  const adminUsers = [
  ...
  window.WC = {
    ...
    riskLobbies, adminUsers, reviewQueue, aiJobs,
    fmtDate(d){ return d.toLocaleDateString('en-US',{ weekday:'short', month:'short', day:'numeric' }); },
    matchById(id){ return matches.find(m => m.id === id); },
  };
})();
