/* ============================================================
   GOLAZO — Leaderboard · My Bets · Wallet · Profile
   ============================================================ */
...
function Leaderboard({ s }) {
  ...
  const scopes = guest ? [['global', 'Global'], ['week', 'This week']] : [['global', 'Global'], ['week', 'This week'], ['tier', 'My tier · Gold']];
  return (
    <div className="page fade-up">
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table className="tbl">
            <thead><tr><th>#</th><th>Player</th><th>Tier</th><th style={{ textAlign: 'right' }}>ROI</th><th style={{ textAlign: 'right' }} className="hide-mobile">Net</th><th style={{ textAlign: 'right' }} className="hide-mobile">W/Settled</th></tr></thead>
            <tbody>
  ...
  return (
    <div className="page fade-up">
      ...
      <div className="row gap-8 wrap-w" style={{ marginBottom: 16 }}>
        {[['all', 'All'], ['open', 'Open & live'], ['won', 'Won'], ['lost', 'Lost']].map(([k, l]) =>
          <button key={k} className={`chip ${f === k ? 'active' : ''}`} onClick={() => setF(k)}>{l}</button>)}
      </div>

      <div className="stack gap-10">
        {list.map((b, i) => {
          const m = WC.matchById(b.mid); if (!m) return null;
...
/* ===================== WALLET ===================== */
function Wallet({ s }) {
  const ICON = { SIGNUP: ['star', 'var(--gold)'], CHECKIN: ['fire', 'var(--gold)'], BET: ['ball', 'var(--text-2)'], SETTLE: ['trophy', 'var(--green)'], REFERRAL: ['users', 'var(--sky)'], MISSION: ['target', 'var(--purple)'] };
  return (
    <div className="page page-narrow fade-up">
      ...
      <div className="panel card-pad-lg" style={{ background: 'linear-gradient(150deg, var(--gold-soft), var(--surface))', borderColor: 'rgba(255,200,61,.25)' }}>
        <div className="row between">
          <div><div className="eyebrow">Available balance</div><div className="display tnum" style={{ fontSize: 48, color: 'var(--gold)', marginTop: 6 }}>{s.points.toLocaleString()}</div></div>
          ...
          return (
            <div key={i} className="row between" style={{ padding: '14px 18px', borderBottom: i < WC.ledger.length - 1 ? '1px solid var(--line)' : 0 }}>
              <div className="row gap-12" style={{ minWidth: 0 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--surface-2)', display: 'grid', placeItems: 'center', flex: 'none' }}><Icon name={icon} size={17} style={{ color }} /></div>
                <div style={{ minWidth: 0 }}><div className="small ellip" style={{ fontWeight: 600 }}>{tx.label}</div><div className="tiny muted">{tx.when}</div></div>
              </div>
  ...
  return (
    <div className="page page-narrow fade-up">
        ...
        <div className="row between wrap gap-16">
          <div className="row gap-16">
            ...
            <div>
              ...
              <div className="row gap-8 mt-8"><span className="badge badge-gold"><Icon name="fire" size={12} fill="var(--gold)" />{me.streak}-day streak</span><span className="badge badge-green">🔥 {me.winStreak} win streak</span></div>
            </div>
          </div>
      ...
      <div className="grid gap-12 mt-16" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(110px,1fr))' }}>
        {[['Balance', s.points.toLocaleString(), 'var(--gold)'], ['ROI', '+' + me.roi + '%', 'var(--green)'], ['Rank', '#' + me.rank, 'var(--sky)'], ['Settled', me.settled, 'var(--text)']].map(([l, v, c]) =>
      ...
      <div className="grid gap-12 mt-16" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))' }}>
        <div className="card card-pad">
          ...
          <div className="row gap-8 mt-12 card-2 card-pad" style={{ borderRadius: 'var(--r-sm)' }}><span className="tnum small grow ellip">golazo.gg/r/alexr</span><Btn variant="primary" size="sm" onClick={() => s.toastMsg('Invite link copied!', 'check')}>Copy</Btn></div>
        </div>
      ...
      <div className="grid gap-12" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))' }}>
        {WC.achievements.map(a => (
          <div key={a.name} className="card card-pad" style={{ opacity: a.unlocked ? 1 : .55, borderColor: a.unlocked ? 'rgba(255,200,61,.25)' : 'var(--line)' }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: a.unlocked ? 'var(--gold-soft)' : 'var(--surface-2)', display: 'grid', placeItems: 'center' }}><Icon name={a.icon} size={20} style={{ color: a.unlocked ? 'var(--gold)' : 'var(--muted)' }} /></div>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        {[['Bet lock reminders', true], ['Bet results', true], ['Streak at risk', true], ['Lobby & borrow alerts', false], ['Hot news', false]].map(([l, on], i) => (
          <div key={l} className="row between" style={{ padding: '14px 18px', borderBottom: i < 4 ? '1px solid var(--line)' : 0 }}>
...
function Toggle({ on: init }) {
  ...
  return (
    <button onClick={() => setOn(!on)} style={{ width: 44, height: 26, borderRadius: 999, background: on ? 'var(--green)' : 'var(--surface-3)', position: 'relative', transition: '.2s' }}>
      <span style={{ position: 'absolute', top: 3, left: on ? 21 : 3, width: 20, height: 20, borderRadius: '50%', background: '#fff', transition: '.2s' }} />
    </button>
  );
}

Object.assign(window, { Leaderboard, MyBets, Wallet, Profile });
