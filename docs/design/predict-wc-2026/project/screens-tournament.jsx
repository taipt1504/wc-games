/* ============================================================
   GOLAZO — Teams · Team detail · Groups · Bracket
   ============================================================ */
  ...
  return (
    <div className="page fade-up">
      ...
      <div className="row gap-8 wrap-w" style={{ marginBottom: 18 }}>
        {confs.map(c => <button key={c} className={`chip ${conf === c ? 'active' : ''}`} onClick={() => setConf(c)}>{c === 'all' ? 'All' : c}</button>)}
      </div>
      <div className="grid gap-12" style={{ gridTemplateColumns: 'repeat(auto-fill,minmax(168px,1fr))' }}>
        {list.map(t => (
          <div key={t.id} className="card card-pad card-hover pointer" onClick={() => s.go('team', { id: t.id })}>
            ...
            <div className="row between mt-4"><span className="tiny muted">FIFA #{t.rank}</span><span className="tiny muted">{t.conf}</span></div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="page page-narrow fade-up">
      ...
      <div className="panel card-pad-lg" style={{ background: `linear-gradient(150deg, ${t.colors[0]}22, var(--bg-2))` }}>
        ...
        <div className="row gap-20 mt-16">
          ...
          <div className="stat"><span className="s-val tnum">{t.gd > 0 ? '+' : ''}{t.gd}</span><span className="s-lbl">Goal diff</span></div>
        </div>
      </div>

      <div className="eyebrow mt-24" style={{ marginBottom: 12, display: 'block' }}>Group {t.group} fixtures</div>
      <div className="stack gap-12">
        {fixtures.map(m => <MatchCard key={m.id} m={m} compact onOpen={() => s.go('match', { id: m.id })} />)}
      </div>

      <div className="eyebrow mt-24" style={{ marginBottom: 12, display: 'block' }}>Squad</div>
      <div className="card card-pad">
        <div className="grid gap-x-16 gap-y-8" style={{ gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: 10 }}>
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="row gap-8 small"><span className="tnum muted" style={{ width: 18 }}>{i + 1}</span><span className="t2 ellip">{t.code} Player {i + 1}</span></div>
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="page fade-up">
      ...
      <div className="grid gap-16" style={{ gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))' }}>
        {WC.GROUPS.map(g => {
          const rows = WC.byGroup(g).slice().sort((a, b) => b.pts - a.pts || b.gd - a.gd || b.gf - a.gf);
          return (
            <div key={g} className="card" style={{ overflow: 'hidden' }}>
              <div className="row between card-pad" style={{ paddingBottom: 10 }}><span className="h3">Group {g}</span><span className="tiny muted">Matchday 1–3</span></div>
              <table className="tbl">
                <thead><tr><th>#</th><th>Team</th><th style={{ textAlign: 'center' }}>P</th><th style={{ textAlign: 'center' }}>GD</th><th style={{ textAlign: 'center' }}>Pts</th></tr></thead>
                <tbody>
                  {rows.map((t, i) => (
                    <tr key={t.id} className={i < 2 ? 'hl' : ''} style={{ cursor: 'pointer' }} onClick={() => s.go('team', { id: t.id })}>
                      ...
                      <td><div className="row gap-8"><Flag team={t} size={22} /><span className="small ellip" style={{ fontWeight: 600 }}>{t.code}</span></div></td>
...
function Bracket({ s }) {
  ...
  const winners = WC.GROUPS.map(g => WC.byGroup(g).slice().sort((a, b) => b.pts - a.pts || b.gd - a.gd)[0]);
  const seconds = WC.GROUPS.map(g => WC.byGroup(g).slice().sort((a, b) => b.pts - a.pts || b.gd - a.gd)[1]);
  const pool = [...winners, ...seconds];
  const pick = (i) => pool[i % pool.length];

  const Match = ({ a, b, score, hot }) => (
    <div className="card card-pad" style={{ padding: '10px 12px', minWidth: 180, borderColor: hot ? 'rgba(255,77,141,.35)' : 'var(--line)' }}>
      {[a, b].map((t, i) => (
        <div key={i} className="row between" style={{ padding: '3px 0' }}>
          <div className="row gap-8" style={{ minWidth: 0 }}><Flag team={t} size={20} /><span className="small ellip" style={{ fontWeight: i === 0 ? 700 : 500 }}>{t.code}</span></div>
          <span className="tnum tiny" style={{ color: i === 0 ? 'var(--green)' : 'var(--muted)' }}>{score ? score[i] : '–'}</span>
        </div>
      ))}
    </div>
  );

  const Col = ({ title, count, start, score }) => (
    <div className="stack" style={{ justifyContent: 'space-around', gap: 14, minWidth: 196 }}>
      ...
      {Array.from({ length: count }).map((_, i) => <Match key={i} a={pick(start + i * 2)} b={pick(start + i * 2 + 1)} score={i === 0 && score ? score : null} hot={i === 0 && title === 'Final'} />)}
    </div>
  );
  ...
  return (
    <div className="page fade-up">
        ...
        <div className="row" style={{ gap: 28, alignItems: 'stretch', minWidth: 1100, padding: '8px 0' }}>
          ...
          <div className="stack center" style={{ minWidth: 140, justifyContent: 'center' }}>
            ...
            <div className="card card-pad" style={{ textAlign: 'center', background: 'linear-gradient(150deg,var(--gold-soft),transparent)', borderColor: 'rgba(255,200,61,.4)' }}>
              ...
              <Flag team={winners[0]} size={40} />
              <div className="small mt-8" style={{ fontWeight: 700 }}>{winners[0].name}</div>
            </div>
          </div>
        </div>
      </div>
      <div className="card card-pad mt-16 row between wrap gap-12" style={{ background: 'linear-gradient(120deg,var(--sky-soft),transparent)' }}>
        <div className="row gap-12"><Pundit size={48} mood="idle" /><div><div style={{ fontWeight: 700 }}>Predict the whole bracket</div><div className="tiny t2">Fill every knockout tie and earn bonus points for each correct round.</div></div></div>
        <Btn variant="primary" size="sm" onClick={() => s.toastMsg('Bracket predictor opening soon!', 'bracket', 'var(--sky)')}>Open predictor</Btn>
      </div>
    </div>
  );
}

Object.assign(window, { Teams, TeamDetail, Groups, Bracket });
