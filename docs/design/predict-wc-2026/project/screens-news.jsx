  ...
  return (
    <div className="page fade-up">
      ...
      <div className="row gap-8 wrap-w" style={{ marginBottom: 18 }}>
        {tags.map(t => <button key={t} className={`chip ${tag === t ? 'active' : ''}`} onClick={() => setTag(t)}>{t}</button>)}
      </div>

      {lead && (
        <div className="panel card-pad-lg card-hover pointer" style={{ marginBottom: 18, background: 'linear-gradient(140deg, var(--sky-soft), transparent)' }} onClick={() => s.go('article', { id: lead.id })}>
          <div className="row between"><span className="badge badge-sky">{lead.tag}</span>{lead.hot && <span className="badge badge-magenta">Trending</span>}</div>
          <h2 className="h2 mt-12" style={{ maxWidth: 640 }}>{lead.title}</h2>
          <p className="t2 mt-8" style={{ maxWidth: 620 }}>{lead.excerpt}</p>
          <div className="row gap-12 mt-16 tiny muted"><span>Source · {lead.src}</span><span>·</span><span>{lead.time}</span><span className="badge badge-muted">✨ AI-assisted</span></div>
        </div>
      )}
...
function Article({ s }) {
  const n = WC.news.find(x => x.id === s.param.id) || WC.news[0];
  return (
    <div className="page page-narrow fade-up">
      <button className="chip" onClick={() => s.back()} style={{ marginBottom: 16 }}><Icon name="chevL" size={14} /> Back to wire</button>
      <span className="badge badge-sky">{n.tag}</span>
      <h1 className="h1 mt-12">{n.title}</h1>
      <div className="row gap-12 mt-12 tiny muted"><span>Source · {n.src}</span><span>·</span><span>{n.time}</span><span className="badge badge-muted">✨ AI-assisted</span></div>
      ...
      {n.match && (
        <div className="card card-pad mt-24 row between wrap gap-12" style={{ background: 'linear-gradient(120deg,var(--green-soft),transparent)' }}>
          <div className="row gap-12"><Icon name="ball" size={20} style={{ color: 'var(--green)' }} /><span style={{ fontWeight: 600 }}>This story is about an upcoming match.</span></div>
...
Object.assign(window, { News, Article });
