/* ============================================================
   GOLAZO — Admin console
   Overview · Tournament · Users (+detail) · Lobby risk (+detail)
   · Review queue · AI pipeline · Audit log
   ============================================================ */
function Admin({ s }) {
  const [tab, setTab] = useState('overview');
  const [detail, setDetail] = useState(null); // { kind:'user'|'risk', id }
  const open = (kind, id) => { setDetail({ kind, id }); window.scrollTo(0, 0); };
  const closeDetail = () => setDetail(null);
  const nav = [
    ['overview', 'Overview', 'gauge'], ['tourney', 'Tournament', 'calendar'], ['users', 'Users', 'users'],
    ['risk', 'Lobby risk', 'shield'], ['review', 'Review queue', 'news'], ['pipeline', 'AI pipeline', 'database'], ['audit', 'Audit log', 'lock'],
  ];
  const goTab = (k) => { setTab(k); setDetail(null); };
    ...
    <div style={{ minHeight: '100vh', position: 'relative', zIndex: 1, display: 'flex' }}>
      ...
      <div className="hide-mobile" style={{ width: 230, borderRight: '1px solid var(--line)', background: 'var(--bg-2)', padding: 18, position: 'sticky', top: 0, height: '100vh' }}>
        ...
        <div className="stack gap-4">
          {nav.map(([k, l, ic]) => (
            <button key={k} className={`nav-i ${tab === k ? 'active' : ''}`} onClick={() => goTab(k)}><Icon name={ic} size={18} />{l}</button>
          ))}
        </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div className="only-mobile row gap-8" style={{ padding: 12, overflowX: 'auto', borderBottom: '1px solid var(--line)' }}>
          {nav.map(([k, l]) => <button key={k} className={`chip ${tab === k ? 'active' : ''}`} onClick={() => goTab(k)}>{l}</button>)}
        </div>
...
function KPI({ v, l, c, sub }) {
  return <div className="card card-pad"><div className="display tnum" style={{ fontSize: 30, color: c }}>{v}</div><div className="s-lbl" style={{ marginTop: 4 }}>{l}</div>{sub && <div className="tiny" style={{ color: c, marginTop: 4 }}>{sub}</div>}</div>;
}
function JobDot({ st }) {
...
/* ===================== OVERVIEW ===================== */
function AdmOverview({ setTab, open }) {
  return (
    <div>
  ...
  return (
    <div className="card card-pad" style={{ borderColor: r.risk === 'High' ? 'rgba(255,90,101,.35)' : 'var(--line)' }}>
      <div className="row between">
        <div className="row gap-10"><Icon name="alert" size={18} style={{ color: `var(--${c === 'muted' ? 'muted' : c})` }} /><span className="mono small" style={{ fontWeight: 700 }}>{r.name}</span><span className="tiny muted">{r.members} members</span></div>
...
function AdmTourney({ s, open }) {
  const [view, setView] = useState('matches'); // matches | teams
  const [matches, setMatches] = useState(() => WC.matches.map(m => ({ ...m, locked: m.status !== 'SCHEDULED' })));
  const [filter, setFilter] = useState('all');
  const [edit, setEdit] = useState(null); // match being score-edited
  const [editOdds, setEditOdds] = useState(null); // match being odds-edited
  ...
  const saveScore = (id, hs, as) => {
    setMatches(ms => ms.map(m => m.id === id ? { ...m, hs, as } : m));
    setEdit(null); s.toastMsg(`Score saved · logged to audit`, 'check', 'var(--gold)');
  };
  const settle = (id) => {
  ...
  const saveOdds = (id, mh, md, ma) => {
    setMatches(ms => ms.map(m => m.id === id ? { ...m, odds: { mh, md, ma } } : m));
    setEditOdds(null); s.toastMsg('Odds updated · open bets unaffected', 'check', 'var(--sky)');
  };
  ...
  return (
    <div>
      ...
      <div className="row gap-8 mb-12" style={{ marginBottom: 16 }}>
        {[['matches', 'Matches'], ['teams', 'Teams & groups']].map(([k, l]) => <button key={k} className={`chip ${view === k ? 'active' : ''}`} onClick={() => setView(k)}>{l}</button>)}
      </div>
      ...
      {view === 'matches' && <>
        ...
        <div className="row between wrap gap-12" style={{ marginBottom: 14 }}>
          <div className="row gap-8 wrap-w">
            {[['all', 'All'], ['LIVE', 'Live'], ['SCHEDULED', 'Scheduled'], ['FINISHED', 'Settled']].map(([k, l]) =>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table className="tbl">
              <thead><tr><th>Match</th><th className="hide-mobile">Kickoff</th><th style={{ textAlign: 'center' }}>Status</th><th style={{ textAlign: 'center' }}>Score</th><th style={{ textAlign: 'center' }} className="hide-mobile">Odds 1·X·2</th><th style={{ textAlign: 'center' }}>Betting</th><th style={{ textAlign: 'right' }}>Actions</th></tr></thead>
              <tbody>
                {list.map(m => {
                  const home = WC.byId(m.home), away = WC.byId(m.away);
                  const sc = m.status === 'SCHEDULED';
                  return (
                    <tr key={m.id} style={{ cursor: 'pointer' }} onClick={() => open && open('match', m.id)}>
                      <td><div className="row gap-8" style={{ minWidth: 0 }}><Flag team={home} size={20} /><span className="small nowrap">{home.code} v {away.code}</span><span className="tiny muted hide-mobile">{m.stage}</span></div></td>
                      <td className="tiny t2 hide-mobile nowrap">{WC.fmtDate(m.date)} · {m.kickoff}</td>
                      <td style={{ textAlign: 'center' }}>
                        {m.status === 'LIVE' ? <span className="badge badge-magenta"><span className="live-dot"></span>{m.minute}'</span>
        ...
        <div className="card-2 card-pad mt-12 small t2 row gap-8" style={{ borderRadius: 'var(--r-sm)' }}>
          <Icon name="alert" size={15} style={{ color: 'var(--gold)', flex: 'none' }} /><span>Settlement is idempotent — re-running never double-pays. Manual score overrides require a reason and are written to the audit log.</span>
        </div>
      </>}
...
function ScoreEditModal({ m, onClose, onSave }) {
  const home = WC.byId(m.home), away = WC.byId(m.away);
    ...
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="card-pad-lg">
          <div className="row between"><span className="eyebrow">Override score</span><button className="btn-icon" onClick={onClose}><Icon name="x" size={18} /></button></div>
          <div className="tiny muted mt-4">{m.stage} · {WC.fmtDate(m.date)}</div>
          <div className="row between center gap-12 mt-16">
            <div className="stack center gap-8" style={{ flex: 1 }}><Flag team={home} size={40} /><span className="small" style={{ fontWeight: 700 }}>{home.code}</span>
              <input className="input input-mono" type="number" min="0" value={hs} onChange={e => setHs(Math.max(0, +e.target.value || 0))} style={{ textAlign: 'center', fontSize: 22, width: 70 }} /></div>
            <span className="display muted" style={{ fontSize: 24 }}>:</span>
            <div className="stack center gap-8" style={{ flex: 1 }}><Flag team={away} size={40} /><span className="small" style={{ fontWeight: 700 }}>{away.code}</span>
              <input className="input input-mono" type="number" min="0" value={as} onChange={e => setAs(Math.max(0, +e.target.value || 0))} style={{ textAlign: 'center', fontSize: 22, width: 70 }} /></div>
          </div>
          <div className="field mt-16"><label className="label">Reason (required for audit)</label><input className="input" placeholder="e.g. API mismatch / VAR correction" value={reason} onChange={e => setReason(e.target.value)} /></div>
  ...
  return (
    <div>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table className="tbl">
            <thead><tr><th>Team</th><th style={{ textAlign: 'center' }}>Group</th><th className="hide-mobile">Confederation</th><th style={{ textAlign: 'center' }}>FIFA</th><th style={{ textAlign: 'center' }}>Squad</th><th style={{ textAlign: 'right' }}></th></tr></thead>
            <tbody>
              {WC.teams.slice(0, 16).map(t => (
                <tr key={t.id} style={{ cursor: 'pointer' }} onClick={() => open && open('team', t.id)}>
                  <td><div className="row gap-8"><Flag team={t} size={20} /><span className="small" style={{ fontWeight: 600 }}>{t.name}</span></div></td>
  ...
  return (
    <div>
      ...
      <div className="row gap-8 card card-pad" style={{ marginBottom: 14, borderRadius: 'var(--r-pill)', maxWidth: 360 }}>
        <Icon name="search" size={16} className="muted" /><input className="input" style={{ border: 0, background: 'transparent', padding: '4px 0' }} placeholder="Search email, username, IP" />
      </div>
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table className="tbl">
            <thead><tr><th>User</th><th className="hide-mobile">IP</th><th style={{ textAlign: 'right' }}>Points</th><th style={{ textAlign: 'center' }}>Flags</th><th style={{ textAlign: 'center' }}>Status</th><th></th></tr></thead>
            <tbody>
              {WC.adminUsers.map((u, i) => (
                <tr key={i} style={{ cursor: 'pointer' }} onClick={() => open('user', i)}>
                  <td><div className="row gap-10"><Avatar initials={u.name.slice(0, 2).toUpperCase()} size={28} color={u.status === 'banned' ? 'var(--danger)' : 'var(--sky)'} /><div><div className="small" style={{ fontWeight: 600 }}>{u.name}</div><div className="tiny muted">{u.email}</div></div></div></td>
                  <td className="tnum tiny t2 hide-mobile">{u.ip}</td>
                  <td className="tnum" style={{ textAlign: 'right' }}>{u.pts.toLocaleString()}</td>
                  <td style={{ textAlign: 'center' }}>{u.flags ? <span className="badge badge-danger">{u.flags}</span> : <span className="muted">—</span>}</td>
                  <td style={{ textAlign: 'center' }}><span className={`badge badge-${u.status === 'active' ? 'green' : u.status === 'banned' ? 'danger' : 'gold'}`}>{u.status}</span></td>
                  <td style={{ textAlign: 'right' }}><Icon name="chevR" size={16} className="muted" /></td>
                </tr>
              ))}
            </tbody>
...
/* ===================== USER DETAIL ===================== */
function AdmUserDetail({ id, onBack, s }) {
  ...
  const act = (msg, icon, color) => s.toastMsg(msg, icon, color);
      ...
      <div className="panel card-pad-lg" style={{ background: 'linear-gradient(160deg, var(--surface-2), var(--bg-2))' }}>
        <div className="row between wrap gap-16">
          <div className="row gap-16">
            ...
            <div>
              <div className="row gap-8"><span className="h3">{u.name}</span>
                <span className={`badge badge-${status === 'active' ? 'green' : status === 'banned' ? 'danger' : 'gold'}`}>{status}</span></div>
              <div className="tiny muted mt-4">{u.email} · joined {u.joined}</div>
              <div className="row gap-8 mt-8"><span className="badge badge-muted mono">{u.ip}</span><span className="badge badge-muted">Chrome / Win</span></div>
            </div>
          </div>
          <div className="row gap-8 wrap-w">
      ...
      <div className="grid gap-16 mt-16" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))' }}>
        ...
        <div className="card card-pad">
          ...
          <div className="stack gap-10 mt-12">
            {ledger.map((x, i) => (
              <div key={i} className="row between small"><div><div className="t2">{x.l}</div><div className="tiny muted">{x.w}</div></div>
                <span className="tnum" style={{ fontWeight: 700, color: x.d > 0 ? 'var(--green)' : 'var(--danger)' }}>{x.d > 0 ? '+' : ''}{x.d}</span></div>
            ))}
          </div>
        </div>
        {/* bets */}
        <div className="card card-pad">
          ...
          <div className="stack gap-10 mt-12">
            {bets.map((b, i) => {
              const m = WC.matchById(b.m); const home = WC.byId(m.home), away = WC.byId(m.away);
              const c = b.status === 'WON' ? 'green' : b.status === 'LOST' ? 'danger' : 'magenta';
              return <div key={i} className="row between"><div className="row gap-8"><Flag team={home} size={18} /><span className="small">{home.code} v {away.code}</span><span className="badge badge-muted">{b.pick}</span></div>
                <div className="row gap-8"><span className="tnum tiny t2">{b.stake}</span><span className={`badge badge-${c}`}>{b.status}</span></div></div>;
            })}
          </div>
        </div>
        {/* linked accounts */}
        <div className="card card-pad" style={{ borderColor: cluster.length ? 'rgba(255,90,101,.3)' : 'var(--line)' }}>
          ...
          {cluster.length ? (
            <div className="stack gap-10 mt-12">
              {cluster.map((c, i) => (
                <div key={i} className="row between"><div className="row gap-8"><Avatar initials={c.name.slice(0, 2).toUpperCase()} size={26} color="var(--danger)" /><span className="small">{c.name}</span></div>
                  <span className="badge badge-danger">{c.flags} flags</span></div>
              ))}
...
/* ===================== LOBBY RISK DETAIL ===================== */
function AdmRiskDetail({ id, onBack, s }) {
    ...
    <div>
      ...
      <div className="panel card-pad-lg" style={{ background: 'linear-gradient(160deg, var(--surface-2), var(--bg-2))' }}>
        <div className="row between wrap gap-12">
          <div>
            <div className="row gap-8"><Icon name="shield" size={18} style={{ color: `var(--${rc === 'muted' ? 'muted' : rc})` }} /><span className="mono h3">{r.name}</span></div>
        ...
        {resolved && <div className="row gap-8 mt-12"><span className={`badge badge-${resolved === 'ban' ? 'danger' : resolved === 'warn' ? 'gold' : 'green'}`}>Resolved · {resolved === 'ban' ? 'Closed & banned' : resolved === 'warn' ? 'Warned' : 'Dismissed'}</span></div>}
      </div>
      ...
      <div className="grid gap-16 mt-16" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))' }}>
        <div className="card card-pad" style={{ borderColor: 'rgba(255,90,101,.35)' }}>
          ...
          <div className="display tnum mt-8" style={{ fontSize: 40, color: `var(--${rc === 'muted' ? 'text' : rc})` }}>{r.score}<span className="muted" style={{ fontSize: 18 }}>/100</span></div>
          <div className="stack gap-8 mt-12">{r.reasons.map((x, i) => <div key={i} className="row gap-8 small t2"><Icon name="alert" size={14} style={{ color: 'var(--danger)' }} />{x}</div>)}</div>
        </div>
        <div className="card card-pad">
          <span className="eyebrow">Point flow</span>
          <div className="stack gap-10 mt-12">
            {[['ghost_07 → ghost_08', '−1,200', 'one-way'], ['ghost_08 → ghost_07', '0', ''], ['borrow ×6', '+1,200', 'maxed']].map(([a, b, t], i) => (
              <div key={i} className="row between small"><span className="t2 mono">{a}</span><div className="row gap-8"><span className="tnum text-danger">{b}</span>{t && <span className="badge badge-danger">{t}</span>}</div></div>
            ))}
          </div>
        </div>
        <div className="card card-pad">
          ...
          <div className="stack gap-8 mt-12">
            {['ghost_07 · 113.161.x.x · Chrome/Win', 'ghost_08 · 113.161.x.x · Chrome/Win'].map((x, i) => <div key={i} className="row gap-8 small t2"><Icon name="alert" size={14} style={{ color: 'var(--gold)' }} /><span className="mono tiny">{x}</span></div>)}
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <table className="tbl">
          <thead><tr><th>Member</th><th style={{ textAlign: 'center' }}>Role</th><th style={{ textAlign: 'right' }}>Net flow</th><th style={{ textAlign: 'right' }}>Borrowed</th><th style={{ textAlign: 'center' }}>Flag</th></tr></thead>
          <tbody>
            {members.map((m, i) => (
              <tr key={i}>
                <td><div className="row gap-8"><Avatar initials={m.name.slice(0, 2).toUpperCase()} size={26} color={m.flag ? 'var(--danger)' : 'var(--sky)'} /><span className="small mono">{m.name}</span></div></td>
                <td style={{ textAlign: 'center' }}><span className="badge badge-muted">{m.role}</span></td>
                <td className="tnum" style={{ textAlign: 'right', color: m.net >= 0 ? 'var(--green)' : 'var(--danger)' }}>{m.net >= 0 ? '+' : ''}{m.net}</td>
                <td className="tnum t2" style={{ textAlign: 'right' }}>{m.borrowed || '—'}</td>
                <td style={{ textAlign: 'center' }}>{m.flag ? <Icon name="alert" size={15} style={{ color: 'var(--danger)' }} /> : <span className="muted">—</span>}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
/* ===================== REVIEW QUEUE ===================== */
function AdmReview({ open }) {
  return (
    <div>
      <SecHead title="News review queue" sub="Tap a story to read the full draft before approving" />
      <div className="stack gap-12">
        {WC.reviewQueue.map(a => (
          <div key={a.id} className="card card-pad card-hover pointer" onClick={() => open && open('news', a.id)}>
            <div className="row between wrap gap-10">
              <div style={{ minWidth: 0 }}>
                <div className="row gap-8"><span className="badge badge-sky">{a.tag}</span>{a.warn && <span className="badge badge-gold">⚠ Verify claims</span>}<span className={`badge badge-${a.status === 'APPROVED' ? 'green' : 'muted'}`}>{a.status}</span></div>
                <div className="h3 mt-8" style={{ fontSize: 16 }}>{a.title}</div>
                <div className="tiny muted mt-4">Source · {a.src} · grounding confidence <span className="tnum" style={{ color: a.conf > 85 ? 'var(--green)' : 'var(--gold)' }}>{a.conf}%</span></div>
              </div>
              <div className="row gap-8" style={{ alignSelf: 'center' }}>
                {a.status === 'PENDING' && <Btn variant="primary" size="sm" icon="eye" onClick={(e) => { e.stopPropagation(); open('news', a.id); }}>Review</Btn>}
  ...
  return (
    <div>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table className="tbl">
            <thead><tr><th>Job</th><th>Provider</th><th style={{ textAlign: 'center' }}>Status</th><th style={{ textAlign: 'right' }} className="hide-mobile">Latency</th><th style={{ textAlign: 'right' }}>Last run</th><th></th></tr></thead>
            <tbody>
              {WC.aiJobs.map((j, i) => (
                <tr key={i}>
                  <td className="mono small">{j.name}</td>
                  <td className="small t2">{j.provider}</td>
                  <td style={{ textAlign: 'center' }}><span className="row gap-6 center"><JobDot st={j.status} /><span className="tiny" style={{ textTransform: 'capitalize' }}>{j.status}</span></span></td>
      ...
      <div className="card-2 card-pad mt-12 small t2 row gap-8" style={{ borderRadius: 'var(--r-sm)' }}>
        <Icon name="alert" size={15} style={{ color: 'var(--gold)', flex: 'none' }} /><span><b>pundit.preview</b> fell back to OpenAI after Claude hit its quota. <b>news.crawl</b> failed (source 503) — retry queued.</span>
      </div>
    </div>
  );
}
        ...
        {log.map((l, i) => (
          <div key={i} className="row between" style={{ padding: '14px 16px', borderBottom: i < log.length - 1 ? '1px solid var(--line)' : 0 }}>
            <div className="row gap-12" style={{ minWidth: 0 }}>
              ...
              <div style={{ minWidth: 0 }}><div className="small ellip">{l[1]}</div>{l[2] !== '—' && <div className="tiny muted">reason · {l[2]}</div>}</div>
            </div>
...
/* ===================== ODDS EDIT MODAL ===================== */
function OddsEditModal({ m, onClose, onSave }) {
  const home = WC.byId(m.home), away = WC.byId(m.away);
  const [mh, setMh] = useState(m.odds.mh);
  const [md, setMd] = useState(m.odds.md);
  const [ma, setMa] = useState(m.odds.ma);
  const [reason, setReason] = useState('');
  const num = (v, set) => <input className="input input-mono" type="number" step="0.01" min="0.1" value={v} onChange={e => set(Math.max(0.1, +e.target.value || 0.1))} style={{ textAlign: 'center', fontSize: 18 }} />;
  const margin = (((1 / (1 + mh)) + (1 / (1 + md)) + (1 / (1 + ma))) * 100 - 100).toFixed(1);
  return (
    <div className="overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="card-pad-lg">
          <div className="row between"><span className="eyebrow">Adjust prediction odds</span><button className="btn-icon" onClick={onClose}><Icon name="x" size={18} /></button></div>
          ...
          <div className="card-2 card-pad mt-16 row between small" style={{ borderRadius: 'var(--r-sm)' }}>
            ...
            <span className="tnum" style={{ fontWeight: 700, color: Math.abs(margin) > 12 ? 'var(--gold)' : 'var(--green)' }}>{margin}%</span>
          </div>
...
/* ===================== ADMIN MATCH DETAIL ===================== */
function AdmMatchDetail({ id, onBack, s }) {
  const base = WC.matchById(id);
  const [m, setM] = useState({ ...base, locked: base.status !== 'SCHEDULED' });
  const [edit, setEdit] = useState(null);
  const [editOdds, setEditOdds] = useState(null);
  const home = WC.byId(m.home), away = WC.byId(m.away);
  const live = m.status === 'LIVE', fin = m.status === 'FINISHED';
  // synthesized bet distribution
  const dist = { '1': 58, 'X': 17, '2': 25 };
  const vol = 184200;
    ...
    <div>
      ...
      <div className="panel card-pad-lg" style={{ background: 'linear-gradient(160deg, var(--surface-2), var(--bg-2))' }}>
        ...
        <div className="row between center" style={{ marginTop: 18 }}>
          <div className="stack center gap-8" style={{ width: 110 }}><Flag team={home} size={52} /><span className="small" style={{ fontWeight: 700 }}>{home.name}</span><span className="tiny muted">#{home.rank}</span></div>
          <div style={{ textAlign: 'center' }}>
            {(live || fin) ? <div className="display tnum" style={{ fontSize: 44 }}>{m.hs}<span className="muted" style={{ fontSize: 24, padding: '0 8px' }}>:</span>{m.as}</div> : <div className="display muted" style={{ fontSize: 28 }}>VS</div>}
            <div className="tiny muted mt-4">{m.venue}</div>
          </div>
          <div className="stack center gap-8" style={{ width: 110 }}><Flag team={away} size={52} /><span className="small" style={{ fontWeight: 700 }}>{away.name}</span><span className="tiny muted">#{away.rank}</span></div>
        </div>
      </div>
      ...
      <div className="card card-pad mt-16 row between wrap gap-12">
        <div className="row gap-8 wrap-w">
          ...
          <button className="chip" onClick={() => setM(x => ({ ...x, locked: !x.locked }))}>
            <Icon name={m.locked ? 'lock' : 'check'} size={14} style={{ color: m.locked ? 'var(--danger)' : 'var(--green)' }} />{m.locked ? 'Betting locked' : 'Betting open'}
          </button>
        </div>
        {m.status !== 'FINISHED' && <Btn variant="primary" size="sm" icon="trophy" onClick={() => { setM(x => ({ ...x, status: 'FINISHED', locked: true, hs: x.hs ?? 0, as: x.as ?? 0 })); s.toastMsg('Match settled · payouts released', 'trophy', 'var(--green)'); }}>Settle match</Btn>}
      </div>
      ...
      <div className="grid gap-16 mt-16" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))' }}>
        ...
        <div className="card card-pad">
          ...
          <div className="row gap-8 mt-12">
            {[['1', home.code, m.odds.mh], ['X', 'Draw', m.odds.md], ['2', away.code, m.odds.ma]].map(([k, l, v]) => (
              <div key={k} className="odds" style={{ cursor: 'default' }}><span className="o-label">{k} · {l}</span><span className="o-val">{v.toFixed(2)}</span></div>
            ))}
          </div>
        ...
        {/* bet distribution */}
        <div className="card card-pad">
          <div className="row between"><span className="eyebrow">Bet distribution</span><span className="tiny muted tnum">{vol.toLocaleString()} pts</span></div>
          <div className="stack gap-10 mt-12">
            {[['1', home.code, dist['1'], 'var(--green)'], ['X', 'Draw', dist['X'], 'var(--sky)'], ['2', away.code, dist['2'], 'var(--magenta)']].map(([k, l, pct, c]) => (
              <div key={k}>
                <div className="row between tiny"><span className="t2">{k} · {l}</span><span className="tnum" style={{ color: c }}>{pct}%</span></div>
      ...
      {edit && <ScoreEditModal m={edit} onClose={() => setEdit(null)} onSave={(mid, hs, as) => { setM(x => ({ ...x, hs, as })); setEdit(null); s.toastMsg('Score saved · logged to audit', 'check', 'var(--gold)'); }} />}
      {editOdds && <OddsEditModal m={m} onClose={() => setEditOdds(null)} onSave={(mid, mh, md, ma) => { setM(x => ({ ...x, odds: { mh, md, ma } })); setEditOdds(null); s.toastMsg('Odds updated', 'check', 'var(--sky)'); }} />}
    </div>
  );
}

/* ===================== ADMIN TEAM DETAIL ===================== */
function AdmTeamDetail({ id, onBack, s }) {
  const t = WC.byId(id);
  ...
  return (
    <div>
        ...
        <div className="row between wrap gap-12">
          <div className="row gap-16">
            ...
            <div>
              ...
              <div className="row gap-8 mt-8"><span className="badge badge-muted">Group {t.group}</span><span className="badge badge-sky">FIFA #{t.rank}</span><span className="badge badge-muted">{t.conf}</span></div>
            </div>
          </div>
          <div className="row gap-8">
      ...
      <div className="grid gap-12 mt-16" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))' }}>
        ...
        <KPI v={t.pts} l="Group points" c="var(--gold)" />
        <KPI v="0" l="Data conflicts" c="var(--green)" />
      </div>
        ...
        <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 8 }}>
          {POS.map((p, i) => (
            <div key={i} className="row between card-2" style={{ borderRadius: 'var(--r-xs)', padding: '7px 10px' }}>
              <div className="row gap-8 small"><span className="tnum muted" style={{ width: 18 }}>{i + 1}</span><span className="t2 ellip">{t.code} Player {i + 1}</span></div>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        {fixtures.map((m, i) => {
          const h = WC.byId(m.home), a = WC.byId(m.away);
          return (
            <div key={m.id} className="row between" style={{ padding: '12px 16px', borderBottom: i < fixtures.length - 1 ? '1px solid var(--line)' : 0 }}>
              <div className="row gap-8"><Flag team={h} size={18} /><span className="small">{h.code} v {a.code}</span><span className="tiny muted">{m.stage}</span></div>
              <div className="row gap-8">{m.status === 'FINISHED' ? <span className="tnum small">{m.hs}-{m.as}</span> : <span className="tiny muted">{WC.fmtDate(m.date)}</span>}<span className={`badge badge-${m.status === 'FINISHED' ? 'green' : m.status === 'LIVE' ? 'magenta' : 'muted'}`}>{m.status === 'FINISHED' ? 'FT' : m.status === 'LIVE' ? 'LIVE' : 'Sched'}</span></div>
            </div>
          );
        })}
      </div>
...
function AdmNewsDetail({ id, onBack, s }) {
  const a = WC.reviewQueue.find(x => x.id === id) || WC.reviewQueue[0];
  const [status, setStatus] = useState(a.status);
  const act = (st, msg, icon, color) => { setStatus(st); s.toastMsg(msg, icon, color); };
  return (
    <div>
      <button className="chip" onClick={onBack} style={{ marginBottom: 16 }}><Icon name="chevL" size={14} /> Back to review queue</button>

      <div className="grid gap-16" style={{ gridTemplateColumns: 'minmax(0,1.6fr) minmax(0,1fr)', alignItems: 'start' }} id="adm-news-grid">
        {/* draft */}
        <div className="panel card-pad-lg">
          <div className="row between"><span className="badge badge-sky">{a.tag}</span><span className={`badge badge-${status === 'APPROVED' ? 'green' : status === 'REJECTED' ? 'danger' : 'muted'}`}>{status}</span></div>
          <h1 className="h2 mt-12">{a.title}</h1>
          <div className="row gap-12 mt-12 tiny muted"><span>Source · {a.src}</span><span>·</span><span className="badge badge-muted">✨ AI draft · Claude</span></div>
        ...
        <div className="stack gap-16">
          <div className="card card-pad">
            ...
            <div className="row between mt-12"><span className="small t2">Confidence</span><span className="tnum" style={{ fontWeight: 700, color: a.conf > 85 ? 'var(--green)' : 'var(--gold)' }}>{a.conf}%</span></div>
            <div className="bar mt-8"><span style={{ width: a.conf + '%', background: a.conf > 85 ? 'var(--green)' : 'var(--gold)' }} /></div>
            {a.warn && <div className="card-2 card-pad mt-12 small t2 row gap-8" style={{ borderRadius: 'var(--r-sm)' }}><Icon name="alert" size={15} style={{ color: 'var(--gold)', flex: 'none' }} /><span>Contains an unverified transfer claim — confirm against a second source before approving.</span></div>}
          </div>
