...
export interface CrawledLineup {
  manager: string;
  formation: string;
  players: CrawledPlayer[];
}
...
export function normalizePosition(raw: string): string {
  ...
  if (p.includes("FORWARD") || p.includes("STRIK"))
    return p.includes("CENTR") ? "CF" : "ST";
...
export function parseLineupJson(raw: string): CrawledLineup {
  ...
  for (const p of arr) {
    ...
    const position =
      typeof p?.position === "string" ? normalizePosition(p.position) : "";
    ...
    players.push({
      name,
      position,
      number: num != null && num >= 1 && num <= 99 ? num : null,
...
export function applyLineupAssignments(
  roster: RosterPlayer[],
  assignments: CrawledPlayer[],
): PlayerUpdate[] {
  ...
  const messages = [
    {
      ...
      content:
        "You are a football data assistant. Output ONLY a JSON object — no prose, no code fences.",
    },
    {
      role: "user" as const,
      content:
...
export async function enrichAndStoreLineup(
  ...
  if (roster.length === 0)
    return { team: team.name, matched: 0, starters: 0, status: "no-roster" };

  const t0 = Date.now();
  let lineup: CrawledLineup = { manager: "", formation: "", players: [] };
  let status = "ok";
  try {
    lineup = await enrichLineup(gateway, {
      name: team.name,
  ...
  if (lineup.players.length === 0)
    return { team: team.name, matched: 0, starters: 0, status };
  ...
  return {
    ...
    starters: updates.filter((u) => u.isStarter).length,
    status,
  };
}
...
export async function enrichAllLineups(
  ...
  { team: string; matched: number; starters: number; status: string }[]
> {
...
export async function crawlAndStoreSquads(
  ...
  { team: string; count: number; starters: number; status: string }[]
> {
  const results: {
    team: string;
    count: number;
  ...
  for (const t of teams) {
    ...
    results.push({
      team: t.name,
      count: lineup.players.length,
      starters: lineup.players.filter((p) => p.starter).length,
      status,
    });
  }
  return results;
