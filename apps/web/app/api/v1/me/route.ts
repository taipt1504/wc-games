...
import { isCheckedInToday, SIGNUP_BONUS } from '@wc/auth';
...
export async function GET() {
  ...
  return NextResponse.json({
    data: {
      ...
      roi: Math.round((netProfit / Number(SIGNUP_BONUS)) * 1000) / 10, // bankroll growth %: net ÷ 1000 signup grant
      rank,
      streak: streakRow?.checkinStreak ?? 0,
      checkedIn: isCheckedInToday(streakRow?.lastCheckinDate),
